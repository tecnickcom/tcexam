...
class phpCAS
{
	...
	public static function error($msg)
		...
		if ( is_array($dbg) ) {
			for ( $i=1; $i<sizeof($dbg); $i++) {
				if ( is_array($dbg[$i]) ) {
					// 2009-02-13 Nicola Asuni - added isset part
					if (isset($dbg[$i]['class']) AND ($dbg[$i]['class'] == __CLASS__ )) {
	 ...
	 */
	public static function setPGTStorageFile($format='',
		$path='')
		{
	 ...
	 */
	public static function setPGTStorageDB($user,
							 $password,
							 $database_type='',
								 $hostname='',
									 $port=0,
	 ...
	 */
	public static function serviceMail($url,$flags,&$err_code,&$err_msg,&$pt)
		{
		global $PHPCAS_CLIENT, $PHPCAS_AUTH_CHECK_CALL;
