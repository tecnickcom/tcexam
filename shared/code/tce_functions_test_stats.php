...
function F_getUserTestStat($test_id, $user_id = 0, $testuser_id = 0, $pubmode = false)
{
    ...
    return $data + F_getUserTestTotals($test_id, $user_id, $testuser_id, $pubmode);
}
 ...
 * return $data array containing test statistics.
 */
function F_getRawTestStat($test_id, $group_id = 0, $user_id = 0, $startdate = 0, $enddate = 0, $testuser_id = 0, $data = [], $pubmode = false)
{
    require_once('../config/tce_config.php');
    ...
    if ($r = F_db_query($sql, $db)) {
        while ($m = F_db_fetch_array($r)) {
            ...
            if (! isset($data['qstats']['module']["'" . $m['module_id'] . "'"]['subject']["'" . $m['subject_id'] . "'"]['question']["'" . $m['question_id'] . "'"])) {
                $data['qstats']['module']["'" . $m['module_id'] . "'"]['subject']["'" . $m['subject_id'] . "'"]['question']["'" . $m['question_id'] . "'"] = [
                    'id' => $m['question_id'],
            ...
            ++$data['qstats']['module']["'" . $m['module_id'] . "'"]['subject']["'" . $m['subject_id'] . "'"]['question']["'" . $m['question_id'] . "'"]['qnum'];
            ...
            if ($ra = F_db_query($sqla, $db)) {
                while ($ma = F_db_fetch_array($ra)) {
                    ...
                    if (! isset($data['qstats']['module']["'" . $m['module_id'] . "'"]['subject']["'" . $m['subject_id'] . "'"]['question']["'" . $m['question_id'] . "'"]['answer']["'" . $ma['answer_id'] . "'"])) {
                        $data['qstats']['module']["'" . $m['module_id'] . "'"]['subject']["'" . $m['subject_id'] . "'"]['question']["'" . $m['question_id'] . "'"]['answer']["'" . $ma['answer_id'] . "'"] = [
                            'id' => $ma['answer_id'],
 ...
 */
function F_printTestStat($test_id, $group_id = 0, $user_id = 0, $startdate = 0, $enddate = 0, $testuser_id = 0, $ts = [], $display_mode = 2, $pubmode = false)
{
    if ($display_mode < 2) {
        return;
    }
...
function F_getAllUsersTestStat($test_id, $group_id = 0, $user_id = 0, $startdate = 0, $enddate = 0, $full_order_field = 'total_score', $pubmode = false, $stats = 2)
{
    ...
    if ($rr = F_db_query($sqlr, $db)) {
        ...
        while ($mr = F_db_fetch_array($rr)) {
            ...
            if ($mr['testuser_end_time'] <= 0 || strtotime($mr['testuser_end_time']) < strtotime($mr['testuser_creation_time'])) {
...
function F_getTestIDs($test_id, $user_id, $filter = 'test_results_to_users')
{
    ...
    $test_id = (int) $test_id;
    $user_id = (int) $user_id;
    $sql = 'SELECT test_id FROM ' . K_TABLE_TESTS . ' WHERE test_id IN (SELECT DISTINCT testuser_test_id FROM ' . K_TABLE_TEST_USER . ' WHERE testuser_user_id=' . (int) $user_id . ' AND testuser_status>3) AND ' . $filter . '=1';
    if ($r = F_db_query($sql, $db)) {
